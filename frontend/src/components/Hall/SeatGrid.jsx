...
const SeatGrid = ({ hallId }) => {
  ...
  const handleReservation = async () => {
    ...
    if (projectionId) {
      payload = {
        ...
        projectionId: parseInt(projectionId, 10),
        seatIds: selected,
        type: ticketType,
      };
  ...
  // ✅ FIX: payment request must include Bearer token + safe JSON handling
  const handlePaymentTwo = async () => {
    const totalAmount = reservedSeats.length * seatPrice;
    const token = localStorage.getItem("token");

    if (!token) {
      setSuccessMessage("You must be logged in to pay.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(text || `Payment request failed (${response.status})`);
      }
      ...
      setSuccessMessage(err.message || "Payment failed. Please try again.");
    }
  };

  // ✅ FIX: sendEmail should also include token (typically protected)
  const sendEmail = async () => {
    const token = localStorage.getItem("token");
    if (!emailInput || !window._reservationId) return;

    try {
      await api.post(`/tickets/email/${window._reservationId}?email=${encodeURIComponent(emailInput)}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setSuccessMessage(err?.response?.data || err.message || "Failed to send email.");
    }
  };
    ...
    <div className="seat-grid-wrapper">
            ...
            {seatsInRow.map((seat) => {
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    if (!seat.taken && !isReserved) toggleSeat(seat.id);
                  }}
                >
                  {String.fromCharCode(64 + seat.rowNumber)}
                  {seat.seatNumber}
                </div>
              );
            })}
      ...
      {selected.length > 0 && (
        <div className="summary enhanced-summary">
          ...
          <p>
            <strong>Selected:</strong> {selected.length} seats
          </p>
          <p>
            <strong>Total:</strong> {(selected.length * seatPrice).toFixed(2)} BAM
          </p>
      ...
      {showPreview && selectedProjection && (
        <div className="ticket-preview">
          <h3>🎟️ Your Ticket</h3>
          <p>
            <strong>Movie:</strong> {selectedProjection.movieTitle}
          </p>
          <p>
            <strong>Hall:</strong> {selectedProjection.hallName}
          </p>
          <p>
            <strong>Time:</strong> {new Date(selectedProjection.startTime).toLocaleString()}
          </p>
          <p>
            <strong>Seats:</strong> {seatLabels}
          </p>
          <p>
            <strong>Total:</strong> {(reservedSeats.length * seatPrice).toFixed(2)} BAM
          </p>
          <button onClick={handlePaymentTwo} className="btn-reserve">
            💳 Pay Now
          </button>
        </div>
      )}
